...
window.P = SS.util.makeSpecifiedArray1D(
  N,
  function() {
    return SS.util.randomInt(0, N - 1);
  },
  new Uint32Array(N)
);
...
SS.util.EditorCamera = function(
  ...
  this.cameraStartPos =
    cameraStartPos || new THREE.Vector2(Math.PI / 8, -Math.PI / 4);
  ...
  var addEventListeners = function() {
    document.addEventListener("mousemove", function(event) {
      ...
      if (editorCamera.mouseDown == true) {
        var diff = mousePos
          .clone()
          .sub(editorCamera.mouseClickPos)
          .multiplyScalar(1 / 500);
  ...
  /*
	function setCameraToRotate() {
		editorCamera.camera.position.y = Math.sin((this.cameraPos.x + Math.PI/2));
		editorCamera.camera.position.x = 1.5 * (Math.cos((this.cameraPos.x + Math.PI/2) * 2)/10) -0.85;
		editorCamera.camera.position.z = 0.7 * (-1 * Math.cos((this.cameraPos.x + Math.PI/2)));
	...
	function setCameraToStartPosition() {
		editorCamera.camera.position.y = 0;
		editorCamera.camera.position.x = -0.7;
		editorCamera.camera.position.z = -0.7;
  ...
  this.rotateCamera = function() {
		if (style == "newspin"){
		// New Spin mechanic
    this.camera.position.y = Math.sin((this.cameraPos.x + Math.PI/2));
    this.camera.position.x = 1.5 * (Math.cos((this.cameraPos.x + Math.PI/2) * 2)/10) -0.85;
    this.camera.position.z = 0.7 * (-1 * Math.cos((this.cameraPos.x + Math.PI/2)));
		// console.log("newspin");
...
// console.log("classicspin");
		...
		newy = ((Math.sin((this.cameraPos.x + Math.PI/2))+1)/10*7)*-1+1;
		if (newy >= 0.99) {
			newy = 1
		}};
		// console.log("entryspin");
...
SS.util.trilinearInterpolation = function(
  coordFloat,
  scalarField,
  interpolation
) {
  ...
  var coord0 = {
    x: Math.floor(coordFloat.x),
    y: Math.floor(coordFloat.y),
    z: Math.floor(coordFloat.z)
  };
  ...
  var c00 = interpolation(
    scalarField(coord0.x, coord0.y, coord0.z),
    scalarField(coord1.x, coord0.y, coord0.z),
    xd
  );
  var c10 = interpolation(
    scalarField(coord0.x, coord1.y, coord0.z),
    scalarField(coord1.x, coord1.y, coord0.z),
    xd
  );
  var c01 = interpolation(
    scalarField(coord0.x, coord0.y, coord1.z),
    scalarField(coord1.x, coord0.y, coord1.z),
    xd
  );
  var c11 = interpolation(
    scalarField(coord0.x, coord1.y, coord1.z),
    scalarField(coord1.x, coord1.y, coord1.z),
    xd
  );
...
SS.util.nearestNeighbour = function(coordFloat, scalarField) {
  return scalarField(
    Math.floor(coordFloat.x),
    Math.floor(coordFloat.y),
    Math.floor(coordFloat.z)
  );
};
